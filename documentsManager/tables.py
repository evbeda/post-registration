...
class SubmissionsTable(tables.Table):
    ...
    def render_state(self, value, record):
        if not self.is_organizer:
            review = Review.objects.filter(submission=record, evaluator=self.evaluator)
            if len(review) == 0:
                value = 'Pending'
            elif review[0].aproved:
                value = 'Accepted'
            else:
                value = 'Rejected'
        badge_style = ''
    ...
    def render_actions(self, value, record):
        ...
        if self.is_organizer:
            action_btn = '<span class="eds-icon-button eds-icon-button--active eds-icon-button--brand"><a href="\
            ' + close_url + '"><button class="eds-btn--button eds-btn--none eds-btn--icon-only" type="button">\
            ...
            action_btn = '<span class="eds-icon-button eds-icon-button--active eds-icon-button--brand"><a href="\
            ' + close_btn + '"><button class="eds-btn--button eds-btn--none eds-btn--icon-only" type="button">\
            <i class="eds-vector-image eds-icon--small" data-spec="icon" aria-hidden="true"><img src=\
            "/static/images/justice-hammer.svg" alt="Hammer"></i></button></a></span>'
        return format_html(
            action_btn + '<span class="eds-icon-button eds-icon-button--active eds-icon-button--brand"><a href="\
            ...
            <i class="eds-vector-image eds-icon--small" data-spec="icon" aria-hidden="true"><svg id="eds-icon--eye_svg"\
             ...
             </path></svg></i></button></a></span>' + '<span class="eds-icon-button eds-icon-button--active \
             eds-icon-button--brand"><a href="' + record.filesubmission.file.url + '">    <button class="\
